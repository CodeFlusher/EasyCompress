...
class InstallationDialog : Initializable {
    ...
    * */
    ...
    private fun initializeLocal() {
        ...
        localInstallationPath.text =
            System.getenv("ProgramFiles").plus(File.separator + "CodeFlusher" + File.separator + "EasyCompress")
    ...
    * */
    ...
    fun onInstall() {
        ...
        Logger.message(
            "Installing",
            "Running icon unpacker:",
            "java -jar " + newFile.absolutePathString() + " iconInstallation"
        )
        ...
        settingsFile.writeText(
            gson.toJson(
                (Settings(
                    localFFmpegPath.text,
                    debug = false,
                    hardwareAcceleration = localUseHWAcceleration.isSelected
                ))
            )
        )
        ...
        if (localCreateShortcut.isSelected) {
            ...
            try {
                ShortcutFactory.createDesktopShortcut(
        ...
        if (localCreateFolder.isSelected) {
            ...
            try {
                ShortcutFactory.createShortcut(
                    startFile.absolutePathString(),
                    startUpFile.absolutePath,
                    iconFile.absolutePathString()
                )
            } catch (e: Exception) {
                Logger.exception("Shortcut", e)
        ...
        //Finishing
        if (Utils.askChooseDialog(
                "Installation Finished",
                "Successfully installed. Open now?",
                ButtonType.YES,
                ButtonType.CLOSE
            ) == ButtonType.YES
        ) {
